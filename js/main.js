const carouselWork = () => {
  ...
  const changeSlide = () => {
    twiceTabs.forEach((tab, i) => {
      ...
      if (htmlTabs[active[current]] == htmlTabs[0]) {
        tab.style.width = "35%";
...
const changeReview = () => {
  ...
  const comments = [
    ...
    {
      text:
        "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Atque molestiae saepe tempore suscipit dolore maxime nihil iste blanditiis corporis, consectetur ab voluptatem expedita, excepturi veniam neque totam, cumque illo porro dolor culpa accusamus nisi quos laudantium sed. ",
      author: "Illo Minima, Company DOLORE",
      img: "././assets/team%20person.png"
    },
    {
      text:
        "Eaque blanditiis, totam voluptate tenetur omnis officiis dolore libero harum corporis ipsum necessitatibus, culpa ratione? Tenetur omnis aperiam obcaecati pariatur veritatis, excepturi odit quos soluta.",
  ...
  items.forEach((item, index) => {
    item.addEventListener("click", () => {
      ...
      let indexActive = items.findIndex(item =>
        item.classList.contains("review__client--active")
      );
